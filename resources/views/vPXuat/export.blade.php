...
<div class="title">
    ...
    <em>Mã Phiếu: {{ $phieuXuat->MaPhieuXuat }} ({{ \Carbon\Carbon::parse($phieuXuat->NgayXuat)->format('d/m/Y') }})</em>
</div>
...
<h4>II, Danh sách linh kiện xuất</h4>
...
<table class="linhkien-table">
    ...
    <tbody>
        @foreach ($phieuXuat->chiTietPhieuXuat as $index => $chiTiet)
            <tr>
                <td>{{ $index + 1 }}</td>
...
<p><em>Ghi chú: </em>{{ $phieuXuat->GhiChu ?? 'Không có ghi chú' }}</p>

<p class="note">
    Tôi xin cam đoan rằng các thông tin nêu trên là đầy đủ và chính xác, và chịu trách nhiệm về tính xác thực của nội dung trong phiếu xuất kho này.
</p>
